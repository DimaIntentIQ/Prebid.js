...
const intentIqIdSubmoduleForTest = intentIqIdSubmodule as any;
const createPixelUrlForTest = createPixelUrl as any;
...
const defaultConfigParams: any = { params: { partner } };
const paiConfigParams: any = { params: { partner, pai } };
const pcidConfigParams: any = { params: { partner, partnerClientIdType, partnerClientId } };
const allConfigParams: any = { params: { partner, pai, partnerClientIdType, partnerClientId, sourceMetaData } };
...
const mockGAM = (): any => {
  const targetingObject: Record<string, any> = {};
  return {
    cmd: [],
    pubads: () => ({
      setTargeting: (key: string, value: any) => {
...
describe('IntentIQ tests', function () {
  ...
  it('should create global IIQ identity object', async () => {
    ...
    const submoduleCallback = intentIqIdSubmoduleForTest.getId({ params: { partner } }).callback;
  ...
  it('should not create a global IIQ identity object in case it was already created', () => {
    intentIqIdSubmoduleForTest.getId({ params: { partner } });
  ...
  it('should log an error if no configParams were passed when getId', function () {
    const submodule = intentIqIdSubmoduleForTest.getId({ params: {} });
  ...
  it('should log an error if partner configParam was not a numeric value', function () {
    const submodule = intentIqIdSubmoduleForTest.getId({ params: { partner: '10' } });
    expect(logErrorStub.calledOnce).to.be.true;
    expect(submodule).to.be.undefined;
  });
  ...
  it('should not save data in cookie if relevant type not set', async function () {
    const callBackSpy = sinon.spy();
    const submoduleCallback = intentIqIdSubmoduleForTest.getId(defaultConfigParams).callback;
    submoduleCallback(callBackSpy);
    await waitForClientHints();
  ...
  it('should save data in cookie if storage type is "cookie"', async function () {
    const callBackSpy = sinon.spy();
    const submoduleCallback = intentIqIdSubmoduleForTest.getId({ ...allConfigParams, enabledStorageTypes: ['cookie'] }).callback;
    submoduleCallback(callBackSpy);
    await waitForClientHints();
  ...
  it('should ignore INVALID_ID and invalid responses in decode', function () {
    expect(intentIqIdSubmoduleForTest.decode('INVALID_ID')).to.equal(undefined);
    expect(intentIqIdSubmoduleForTest.decode('')).to.equal(undefined);
    expect(intentIqIdSubmoduleForTest.decode(undefined)).to.equal(undefined);
  });

  it('should send AT=20 request and send source in it', async function () {
    const usedBrowser = 'chrome';
    intentIqIdSubmoduleForTest.getId({
      params: {
        partner: 10,
  ...
  it('should call the IntentIQ endpoint with only partner, pai', async function () {
    const callBackSpy = sinon.spy();
    const submoduleCallback = intentIqIdSubmoduleForTest.getId(paiConfigParams).callback;
    submoduleCallback(callBackSpy);
    await waitForClientHints();
  ...
  it('should call the IntentIQ endpoint with only partner, pcid', async function () {
    const callBackSpy = sinon.spy();
    const submoduleCallback = intentIqIdSubmoduleForTest.getId(pcidConfigParams).callback;
    submoduleCallback(callBackSpy);
    await waitForClientHints();
  ...
  it('should call the IntentIQ endpoint with partner, pcid, pai', async function () {
    const callBackSpy = sinon.spy();
    const submoduleCallback = intentIqIdSubmoduleForTest.getId(allConfigParams).callback;
    submoduleCallback(callBackSpy);
    await waitForClientHints();
    const request = server.requests[0];
    expect(request.url).to.contain('https://api.intentiq.com/profiles_engine/ProfilesEngineServlet?at=39&mi=10&dpi=10&pt=17&dpn=1&pai=11&iiqidtype=2&iiqpcid=');
  ...
  it('should set GAM targeting to B when server tc=41', async () => {
    ...
    const cb = intentIqIdSubmoduleForTest.getId(defaultConfigParams).callback;
    cb(() => {});
    await waitForClientHints();
  ...
  it('should NOT call GAM setTargeting when current browser is in browserBlackList', function () {
    ...
    intentIqIdSubmoduleForTest.getId(cfg);
  ...
  it('return data stored in local storage ', function () {
    ...
    const returnedValue = intentIqIdSubmoduleForTest.getId(allConfigParams);
  ...
  it('should handle browser blacklisting', function () {
    ...
    const submoduleCallback = intentIqIdSubmoduleForTest.getId(configParamsWithBlacklist);
    expect(logErrorStub.calledOnce).to.be.true;
  ...
  it('should save spd to firstPartyData in localStorage if present in response', async function () {
    ...
    const parsedLs = JSON.parse(storedLs as string);
  ...
  describe('detectBrowserFromUserAgentData', function () {
    ...
    it("Should call the server for new partner if FPD has been updated by other partner, and 72 hours have not yet passed.", async () => {
      const allowedStorage: any = ['html5'];
      ...
      const callBackSpy = sinon.spy();
      const submoduleCallback = intentIqIdSubmoduleForTest.getId({ ...allConfigParams, params: { ...allConfigParams.params, partner: newPartnerId } }).callback;
      submoduleCallback(callBackSpy);
      await waitForClientHints();
      const request = server.requests[0];
    ...
    it("Should NOT call the server if FPD has been updated user Opted Out, and 72 hours have not yet passed.", async () => {
      ...
      storeData(FIRST_PARTY_KEY, JSON.stringify(FPD), allowedStorage, FPD);
      const returnedObject = intentIqIdSubmoduleForTest.getId({ ...allConfigParams, params: { ...allConfigParams.params, partner: newPartnerId } });
      await waitForClientHints();
    ...
    it('should NOT call the server for opted-out user when partner data has no cttl (e.g. only terminationCause stored)', async function () {
      ...
      intentIqIdSubmoduleForTest.getId(defaultConfigParams);
      await waitForClientHints();
  ...
  describe('IntentIQ consent management within getId', function () {
    let uspDataHandlerStub: any;
    let gppDataHandlerStub: any;
    let gdprDataHandlerStub: any;
    let uspData: any;
    let gppData: any;
    let gdprData: any;

    function mockConsentHandlers(usp: any, gpp: any, gdpr: any): void {
    ...
    it('should set isOptOut to true for new users if GDPR is detected and update it upon receiving a server response', async function () {
      ...
      const lsBeforeReq = JSON.parse(localStorage.getItem(FIRST_PARTY_KEY) as string);
      ...
      const updatedFirstPartyData = JSON.parse(localStorage.getItem(FIRST_PARTY_KEY) as string);
    ...
    it('should save cmpData parameters in LS data and used it request if uspData, gppData, gdprData exists', async function () {
      ...
      const lsFirstPartyData = JSON.parse(localStorage.getItem(FIRST_PARTY_KEY) as string);
    ...
    it('should keep only terminationCause in partner data, drop identifiers from FPD, and clear client hints when isOptedOut is true in response', async function () {
      ...
      const lsFirstPartyData = JSON.parse(localStorage.getItem(FIRST_PARTY_KEY) as string);
      const lsPartnerData = JSON.parse(localStorage.getItem(FIRST_PARTY_DATA_KEY) as string);
    ...
    it('should not persist pcid/pcidDate/pid/abTestUuid to device when first-party data has isOptedOut=true on a subsequent session', async function () {
      ...
      const lsFPD = JSON.parse(localStorage.getItem(FIRST_PARTY_KEY) as string);
    ...
    it('should make request to correct address with iiqServerAddress parameter', async function() {
      ...
      const callBackSpy = sinon.spy();
      const submoduleCallback = intentIqIdSubmoduleForTest.getId({ ...customParams }).callback;
    ...
    it('should make request to correct address with iiqPixelServerAddress parameter', async function() {
      ...
      intentIqIdSubmoduleForTest.getId({ ...callbackConfigParams });
      await waitForClientHints();
    ...
    regionCases.forEach(({ name, region, expected }) => {
      it(`should use region-specific api endpoint when region is "${name}"`, async function () {
        ...
        const submoduleCallback = intentIqIdSubmoduleForTest.getId(configWithRegion).callback;
        submoduleCallback(callBackSpy);
        await waitForClientHints();
  ...
  it('should add clientHints to the URL if provided', function () {
    ...
    const partnerData = {};
    const cmpData = {};
    const url = createPixelUrlForTest(firstPartyData, clientHints, configParams, partnerData, cmpData);
  ...
  it('should not add clientHints to the URL if not provided', function () {
    ...
    const partnerData = {};
    const cmpData = {};
    const url = createPixelUrlForTest(firstPartyData, undefined, configParams, partnerData, cmpData);
  ...
  it('should sends uh from LS immediately and later updates LS with fresh CH', async () => {
    ...
    const submoduleCallback = intentIqIdSubmoduleForTest.getId(cfg).callback;
    submoduleCallback(callBackSpy);
  ...
  it('should clear CLIENT_HINTS from LS and NOT send uh when CH are not supported', async function () {
    ...
    intentIqIdSubmoduleForTest.getId(cfg).callback(cbSpy);
    await waitForClientHints();
  ...
  describe('appendCMPData via createPixelUrl', function () {
    ...
    it('should not include us_privacy in URL when uspString is null', function () {
      ...
      const url = createPixelUrlForTest({}, undefined, baseParams, {}, cmpData);
  ...
  it('should run callback from params', async () => {
    ...
    await intentIqIdSubmoduleForTest.getId(callbackConfigParams);
    expect(wasCallbackCalled).to.equal(true);
  });
  ...
  it('should NOT send sourceMetaData and sourceMetaDataExternal in AT=39 if it is undefined', async function () {
    ...
    const submoduleCallback = intentIqIdSubmoduleForTest.getId(configParams).callback;
    submoduleCallback(callBackSpy);
    await waitForClientHints();
  ...
  it('should send sourceMetaData in AT=20 if it exists in configParams', async function () {
    ...
    intentIqIdSubmoduleForTest.getId(configParams);
    await waitForClientHints();
    const request = server.requests[0];
  ...
  it('should increment callCount when valid eids are returned', async function () {
    ...
    const updatedData = JSON.parse(localStorage.getItem(firstPartyDataKey) as string);
